...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(self.data[idx]) > 0:
            ...
            document = document[:random.randint(min(4, len(document)), min(int(self.block_size*7/8), len(document)))]

            len_mask = int(random.triangular(0, len(document), 3*(.25*len(document)) - 0 - len(document)))
            prefix = document[:random.randint(0, len(document) - len_mask - 1)]
            masked_content = document[len(prefix):len(prefix) + len_mask]
            suffix = document[len(prefix) + len_mask:]
            ...
            masked_string = self.MASK_CHAR + self.MASK_CHAR + self.PAD_CHAR * (self.block_size-2)
